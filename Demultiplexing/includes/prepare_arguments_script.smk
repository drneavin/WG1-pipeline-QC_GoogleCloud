...
##############################################################
############ Prepare document with file locations ############
##############################################################
rule prepare_args:
    input:
        input_dict["scRNAseq_dir"],
        input_dict["individual_list_dir"],
    ...
    run:
        ...
        scrnaseq_dir = input[0]
        individual_list_dir = input[1]
        ...
        samples = pd.read_csv(input[2], sep = "\t")
        ...
        try:
            ...
            print("Could not find a scRNA-seq directory for all of the pools in your pool list. Please check that they are spelled correctly and you do not have any additional pool names that are not in  " + individual_list_dir)
        ...
        dataframe=pd.concat([scrnaseq_libs.replace("^" + params[0] + "/", "", regex=True, ), barcode_libs.replace("^" + params[0] + "/", "", regex=True, ), bamlibs.replace("^" + params[0] + "/", "", regex=True, ), matrix_libs.replace("^" + params[0] + "/", "", regex=True, ), matrix_dir_libs.replace("^" + params[0] + "/", "", regex=True, ), individual_libs.replace("^" + params[0] + "/", "", regex=True, )], axis=1)

        print(scrnaseq_libs)
        print(scrnaseq_libs.replace("^" + params[0] + "/", "", regex=True))

        print(dataframe)

        dataframe.to_csv(output[0], sep = "\t", index = True)
